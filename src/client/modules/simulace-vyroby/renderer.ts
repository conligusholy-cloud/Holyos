...
import { Token, DrawingObject, Point } from '../../../shared/types.js';
...
export function initDom(): void {
  dom = {
    svg: document.getElementById('canvas') as SVGSVGElement | null,
    container: document.getElementById('canvas-container') as HTMLElement | null,
    objectLayer: document.getElementById('object-layer') as SVGGElement | null,
    connectionLayer: document.getElementById('connection-layer') as SVGGElement | null,
    animationLayer: document.getElementById('animation-layer') as SVGGElement | null,
    gridRect: document.getElementById('grid-rect') as SVGElement | null,
    zoomDisplay: document.getElementById('zoom-display') as HTMLElement | null,
    coordsDisplay: document.getElementById('coords-display') as HTMLElement | null,
  };
}
...
export function screenToWorld(sx: number, sy: number): WorldCoord {
...
export function worldToScreen(wx: number, wy: number): WorldCoord {
  return {
...
export function updateTransform(): void {
  if (!dom.objectLayer || !dom.connectionLayer || !dom.animationLayer) return;
  ...
  const gs5 = gs * 5;
  const gridSmall = document.getElementById('grid-small') as SVGElement | null;
  const gridLarge = document.getElementById('grid-large') as SVGElement | null;
  if (gridSmall && gridLarge) {
    gridSmall.setAttribute('width', String(gs));
    gridSmall.setAttribute('height', String(gs));
    const gridPath = gridSmall.querySelector('path') as SVGElement | null;
    if (gridPath) gridPath.setAttribute('d', `M ${gs} 0 L 0 0 0 ${gs}`);
    gridLarge.setAttribute('width', String(gs5));
    gridLarge.setAttribute('height', String(gs5));
    const gridRect = gridLarge.querySelector('rect') as SVGElement | null;
    if (gridRect) {
      gridRect.setAttribute('width', String(gs5));
      gridRect.setAttribute('height', String(gs5));
    }
    const gridPathLarge = gridLarge.querySelector('path') as SVGElement | null;
    if (gridPathLarge) gridPathLarge.setAttribute('d', `M ${gs5} 0 L 0 0 0 ${gs5}`);
    if (dom.gridRect) {
...
export function renderAll(): void {
  ...
  if (state.arealObjects && state.arealObjects.length > 0) {
    const arealGroup = svgEl('g') as SVGGElement;
...
function renderBackgroundObject(parent: SVGGElement, obj: DrawingObject): void {
  ...
  if (obj.points && obj.points.length >= 3) {
    ...
    // Entrances
    if (obj.entrances) obj.entrances.forEach(ent => renderEntranceSimple(g, obj, ent));
    // Walls
    if (obj.walls) obj.walls.forEach(wall => renderWallSimple(g, wall));
  } else {
...
function addLabel(g: SVGGElement, obj: DrawingObject): void {
  const padding = 0.2;
...
function renderConnections(): void {
  if (!state.connections || !dom.connectionLayer) return;
...
function renderRoutePath(): void {
  if (!state.route || state.route.length < 2 || !dom.objectLayer) return;

  const mappedOps = state.route.filter(op => (op as any).floorX != null);
  if (mappedOps.length < 2) return;
  ...
  for (let i = 0; i < mappedOps.length - 1; i++) {
    ...
    // Šipka
    const line = svgEl('line') as SVGLineElement;
    line.setAttribute('x1', String((from as any).floorX));
    line.setAttribute('y1', String((from as any).floorY));
    line.setAttribute('x2', String((to as any).floorX));
    line.setAttribute('y2', String((to as any).floorY));
  ...
  if (dom.svg && !document.getElementById('arrow-marker')) {
    let defs = dom.svg.querySelector('defs') as SVGDefsElement | null;
    if (!defs) {
...
export function renderTokens(): void {
  if (!dom.animationLayer) return;

  dom.animationLayer.innerHTML = '';

  state.tokens.forEach(token => {
    if (token.state === 'done' && !(token as any).visible) return;
    ...
    if (token.state === 'processing') {
      const pulse = svgEl('circle') as SVGCircleElement;
      ...
      const anim = svgEl('animate') as SVGAnimateElement;
      ...
      const anim2 = svgEl('animate') as SVGAnimateElement;
...
export function highlightStation(obj: DrawingObject, active: boolean): void {
  const gEl = dom.objectLayer?.querySelector(`[data-id="${obj.id}"]`) as SVGGElement | null;
  if (!gEl) return;
...
function getObjectCenter(obj: DrawingObject): WorldCoord {
  if (obj.points && obj.points.length >= 3) {
    let cx = 0, cy = 0;
...
function getPolygonBBox(pts: Point[]): BBox {
...
function renderEntranceSimple(g: SVGGElement, obj: DrawingObject, ent: any): void {
  if (!obj.points) return;
  const i = ent.edgeIndex;
  if (i < 0 || i >= obj.points.length) return;
...
function renderWallSimple(g: SVGGElement, wall: any): void {
  const line = svgEl('line') as SVGLineElement;
...
export function zoomFit(): void {
  ...
  if (!isFinite(minX) || !dom.container) return;

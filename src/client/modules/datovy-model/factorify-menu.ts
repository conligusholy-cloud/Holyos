...
export interface MenuItem {
  label: string;
  slug: string;        // URL slug from Factorify (/ui/{slug})
  entityName: string;  // Factorify entity name for API queries ('' = view/report, no direct query)
}

export interface MenuCategory {
  name: string;        // Category header (PRODEJ, NÁKUP, etc.)
  items: MenuItem[];
}
...
export function buildEntityResolver(apiEntities: { name: string; endpointUrl: string; label: string }[]): Map<string, string> {

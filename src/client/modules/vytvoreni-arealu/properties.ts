...
import { updateProp, updateColor, rotatePolygon, removeEntrance, updateEntranceProp, updateEntranceWidth, removeWall, removeGate, removeRoomLabel, updateRoomLabelProp, duplicateObject, deleteObject } from './objects.js';
...
export function showProperties(id: number): void {
    ...
    if (obj.type === 'hala') {
      ...
      <div class="prop-group">
        ...
        ${walls.length > 0 ? walls.map(wall => {
          ...
          const isHighlighted = (state as any).highlightedWallId === wall.id;
          const rowStyle = [
    ...
    <div class="prop-group">
      ...
      <div class="prop-color">
        ${COLOR_SWATCHES.map(c =>
          `<div class="prop-color-swatch ${obj.color === c ? 'active' : ''}" style="background:${c}" onclick="window.editorAPI.updateColor('${c}')"></div>`
        ).join('')}
      </div>
    </div>`;
  ...
  const hlId = (state as any).highlightedWallId;
  if (hlId != null) {
    setTimeout(() => {
      const row = propsPanel.querySelector(`[data-wall-id="${hlId}"]`) as HTMLElement | null;
      if (row) {
...
export function getRotateCenter(): number {
  const sel = document.getElementById('rotate-center') as HTMLSelectElement;
  return sel ? parseInt(sel.value) : -1;
}

// ---- Editace vertexů z properties panelu ----

export function updateVertex(objId: number, index: number, axis: string, value: number): void {
  pushUndo();
  const obj = state.objects.find(o => o.id === objId);
  if (!obj || !obj.points || !obj.points[index]) return;
  (obj.points[index] as unknown as Record<string, number>)[axis] = value;
  const bbox = getPolygonBBox(obj.points);
  obj.x = bbox.minX;
  obj.y = bbox.minY;
...
export function updateGateProp(objId: number, wallId: number, gateId: number, key: string, value: unknown): void {
  pushUndo();
  const obj = state.objects.find(o => o.id === objId);
  ...
  if (!gate) return;
  (gate as unknown as Record<string, unknown>)[key] = value;
  renderAll();
  showProperties(objId);
}

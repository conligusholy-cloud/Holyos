/* ============================================
   factorify-api.ts — Pracovní postup
...
export const FactorifyAPI = {
  ...
  // Cache
  products: [] as WorkProcedureProduct[],
  allItems: [] as WorkProcedureProduct[],
  stages: [] as StageItem[],
  ...
  // ---- Načíst zboží (Goods) ----
  async loadProducts(): Promise<WorkProcedureProduct[]> {
    this.loading = true;
    this.error = null;
  ...
  async loadStages(): Promise<StageItem[]> {
    try {

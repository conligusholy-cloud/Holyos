...
(function () {
  ...
  $('#prc-refresh').addEventListener('click', () => loadTab(currentTab()));
  ...
  window.__pracacekCloseModal = closeModal;
    ...
    openModal(`
      ...
      <form id="form-new-task">
        ...
        <div class="modal-actions">
          <button type="button" class="btn btn-secondary" onclick="__pracacekCloseModal()">Zrušit</button>
  ...
  async function openActivationModal() {
    ...
    openModal(`
      ...
      <p style="color:var(--text2); font-size:13px; margin-bottom:12px;">Vyber kolegu — vygenerujeme jednorázový kód, který kolega zadá v Pracáčkovi spolu s vlastním PIN.</p>
      <div class="form-group full">
        <label>Osoba</label>
      ...
      <div class="modal-actions">
        <button class="btn btn-secondary" onclick="__pracacekCloseModal()">Zavřít</button>
  ...
  async function loadFences() {
    ...
    try {
      ...
      if (fences.length === 0) {
        wrap.innerHTML = `<div class="empty-state"><div class="empty-icon">🗺️</div><h3>Žádné provozy</h3><p>Definuj GPS zónu kolem provozu — Pracáček podle ní pozná, že kolega přišel do práce.</p></div>`;
        return;
      }
      wrap.innerHTML = `
        <table class="data-table">

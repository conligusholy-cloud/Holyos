/* ============================================
   properties.js — Panel vlastností
   ============================================ */

function showProperties(id) {
  const obj = state.objects.find(o => o.id === id);
  if (!obj) return;
  dom.propsPanel.className = '';
  ...
  html += `
    <div style="display:flex; align-items:center; gap:6px; margin-bottom:10px;">
      <button class="btn btn-icon ${isLocked ? 'btn-lock-active' : ''}" onclick="toggleLock(${id})" title="${isLocked ? 'Odemknout' : 'Zamknout'}" style="font-size:16px; flex-shrink:0;">
        ${isLocked ? '&#128274;' : '&#128275;'}
      </button>
    ...
    <div class="prop-group">
      <label>Název</label>
      <input type="text" value="${obj.name}" onchange="updateProp('name', this.value)" ${isLocked ? 'disabled style="opacity:0.5"' : ''}>
    </div>`;
  ...
  if (isPolygon) {
    ...
    <div class="prop-group">
      ...
      <div style="display:flex; gap:6px; align-items:center;">
        <button class="btn btn-small" onclick="rotatePolygon(${id}, -90, getRotateCenter())" title="−90°" style="width:auto!important;padding:0 8px!important;font-size:13px!important;">↺ 90°</button>
        <button class="btn btn-small" onclick="rotatePolygon(${id}, -45, getRotateCenter())" title="−45°" style="width:auto!important;padding:0 8px!important;font-size:13px!important;">↺ 45°</button>
        <input type="number" value="0" step="5" id="rotate-custom"
          ...
          onkeydown="if(event.key==='Enter'){rotatePolygon(${id}, parseFloat(this.value), getRotateCenter());this.value='0';}">
        ...
        ${obj.points.map((p, i) => {
          ...
          <div class="vertex-row">
            ...
            <input type="number" value="${p.x.toFixed(1)}" step="0.5"
              onchange="updateVertex(${id}, ${i}, 'x', parseFloat(this.value))">
            <input type="number" value="${p.y.toFixed(1)}" step="0.5"
              onchange="updateVertex(${id}, ${i}, 'y', parseFloat(this.value))">
            ${obj.points.length > 3 ? `<button class="btn btn-icon btn-small" onclick="removeVertex(${id}, ${i})" title="Odebrat bod">×</button>` : ''}
          </div>
          <div class="edge-dist-row">
            <span class="edge-arrow">${i + 1}→${j + 1}</span>
            <input type="number" value="${dist.toFixed(1)}" step="0.5" min="0.1"
              onchange="updateEdgeDistance(${id}, ${i}, parseFloat(this.value))">
    ...
    <div class="prop-group">
        ...
        <div class="entrance-card">
          <div class="entrance-card-header">
            ...
            <input type="text" value="${ent.name}" style="flex:1;padding:4px 6px;font-size:12px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:4px;outline:none;"
              onchange="updateEntranceProp(${id}, ${ent.id}, 'name', this.value)">
            <button class="btn btn-danger" onclick="removeEntrance(${id}, ${ent.id})" title="Smazat vjezd" style="padding:4px 8px;font-size:12px;font-weight:600;">✕</button>
          </div>
          <div class="entrance-card-body">
            <div style="display:flex;align-items:center;gap:6px;">
              ...
              <select style="flex:1;padding:3px 6px;font-size:11px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:4px;outline:none;"
                onchange="updateEntranceProp(${id}, ${ent.id}, 'type', this.value)">
            ...
            <div style="display:flex;align-items:center;gap:6px;">
              ...
              <input type="number" value="${width}" step="0.5" min="0.5" style="width:60px;padding:3px 6px;font-size:11px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:4px;outline:none;"
                onchange="updateEntranceWidth(${id}, ${ent.id}, parseFloat(this.value))">
      ...
      <div style="display:flex;gap:4px;margin-top:6px;">
        <button class="btn" onclick="startEntrancePlacement(${id}, 'vjezd')" style="flex:1;font-size:11px;padding:5px 6px;">+ Vjezd</button>
        <button class="btn" onclick="startEntrancePlacement(${id}, 'vyjezd')" style="flex:1;font-size:11px;padding:5px 6px;">+ Výjezd</button>
      </div>
    </div>`;
      ...
      <div class="prop-group">
        ...
        ${walls.length > 0 ? walls.map(wall => {
          ...
          <div class="wall-row">
            ...
            <button class="btn btn-small" onclick="startGatePlacement(${id}, ${wall.id})" title="Přidat vrata" style="width:auto!important;padding:0 6px!important;font-size:11px!important;">+ Vrata</button>
            <button class="btn btn-icon btn-small btn-danger" onclick="removeWall(${id}, ${wall.id})" title="Smazat">×</button>
          </div>
          ${wall.gates && wall.gates.length > 0 ? wall.gates.map(gate => `
            <div class="gate-row">
              ...
              <input type="text" value="${gate.name}" style="flex:1;padding:3px 5px;font-size:11px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:4px;outline:none;"
                onchange="updateGateProp(${id}, ${wall.id}, ${gate.id}, 'name', this.value)">
              <input type="number" value="${gate.width}" step="0.5" min="0.5" style="width:45px;padding:3px 4px;font-size:11px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:4px;outline:none;"
                onchange="updateGateProp(${id}, ${wall.id}, ${gate.id}, 'width', parseFloat(this.value))">
              <span style="font-size:10px;color:var(--text2);">m</span>
              <button class="btn btn-icon btn-small btn-danger" onclick="removeGate(${id}, ${wall.id}, ${gate.id})" title="Odebrat">×</button>
            </div>
          `).join('') : ''}`;
        }).join('') : '<div style="font-size:12px;color:var(--text2);margin-bottom:6px;">Zatím žádné stěny</div>'}
        <button class="btn" onclick="startWallDrawMode(${id})" style="width:100%;margin-top:6px;font-size:12px;">Kreslit stěnu</button>
      </div>`;
          ...
          ${rooms.length > 0 ? rooms.map(room => `
            <div class="wall-row" style="gap:4px;">
              ...
              <input type="text" value="${room.name}" style="flex:1;padding:3px 5px;font-size:11px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:4px;outline:none;"
                onchange="updateRoomLabelProp(${id}, ${room.id}, 'name', this.value)">
              <button class="btn btn-icon btn-small btn-danger" onclick="removeRoomLabel(${id}, ${room.id})" title="Odebrat">×</button>
            </div>
          `).join('') : '<div style="font-size:12px;color:var(--text2);margin-bottom:6px;">Zatím žádné místnosti</div>'}
          <button class="btn" onclick="startRoomLabelPlacement(${id})" style="width:100%;margin-top:6px;font-size:12px;">+ Místnost</button>
        </div>`;
    }
  } else {
    ...
    <div class="prop-row">
      <div class="prop-group">
        <label>Šířka (m)</label>
        <input type="number" value="${obj.w}" step="0.5" min="0.5" onchange="updateProp('w', parseFloat(this.value))">
      </div>
      <div class="prop-group">
        <label>Výška (m)</label>
        <input type="number" value="${obj.h}" step="0.5" min="0.5" onchange="updateProp('h', parseFloat(this.value))">
      </div>
    </div>
    <div class="prop-row">
      <div class="prop-group">
        <label>Pozice X (m)</label>
        <input type="number" value="${obj.x.toFixed(1)}" step="0.5" onchange="updateProp('x', parseFloat(this.value))">
      </div>
      <div class="prop-group">
        <label>Pozice Y (m)</label>
        <input type="number" value="${obj.y.toFixed(1)}" step="0.5" onchange="updateProp('y', parseFloat(this.value))">
      </div>
    </div>
    <div class="prop-group">
      <label>Plocha</label>
    ...
    <div class="prop-group">
      ...
      <div style="display:flex; gap:6px; align-items:center; flex-wrap:wrap;">
        <button class="btn btn-small" onclick="rotateRect(${id}, -90)" title="−90°" style="width:auto!important;padding:0 8px!important;font-size:13px!important;">↺ 90°</button>
        <button class="btn btn-small" onclick="rotateRect(${id}, -45)" title="−45°" style="width:auto!important;padding:0 8px!important;font-size:13px!important;">↺ 45°</button>
        <input type="number" value="${obj.rotation || 0}" step="5" style="width:60px;padding:4px 6px;font-size:12px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:6px;outline:none;"
          onchange="setRectRotation(${id}, parseFloat(this.value))">
        <span style="font-size:11px;color:var(--text2);">°</span>
        <button class="btn btn-small" onclick="rotateRect(${id}, 45)" title="+45°" style="width:auto!important;padding:0 8px!important;font-size:13px!important;">45° ↻</button>
        <button class="btn btn-small" onclick="rotateRect(${id}, 90)" title="+90°" style="width:auto!important;padding:0 8px!important;font-size:13px!important;">90° ↻</button>
      </div>
    ...
    <div class="prop-group">
      ...
      <div class="prop-color">
        ${COLOR_SWATCHES.map(c =>
          `<div class="prop-color-swatch ${obj.color === c ? 'active' : ''}" style="background:${c}" onclick="updateColor('${c}')"></div>`
        ).join('')}
      </div>
    </div>`;
  ...
  html += `
    <div style="margin-top: 20px; display: flex; gap: 6px;">
      <button class="btn" onclick="duplicateObject(${id})" style="flex:1">Duplikovat</button>
      <button class="btn btn-danger" onclick="deleteObject(${id})" style="flex:1" ${isLocked ? 'disabled style="flex:1;opacity:0.4;pointer-events:none;"' : ''}>Smazat</button>
    </div>`;

  dom.propsPanel.innerHTML = html;
}
...
function updateVertex(objId, index, axis, value) {
  pushUndo();
  const obj = state.objects.find(o => o.id === objId);
  if (!obj || !obj.points || !obj.points[index]) return;
...
function updateEdgeDistance(objId, fromIndex, newDist) {
  pushUndo();
  const obj = state.objects.find(o => o.id === objId);
...
function removeVertex(objId, index) {
  pushUndo();
  const obj = state.objects.find(o => o.id === objId);
...
function updateGateProp(objId, wallId, gateId, key, value) {
  pushUndo();
  const obj = state.objects.find(o => o.id === objId);

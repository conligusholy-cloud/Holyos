...
function finishWall(endPoint) {
  ...
  if (dist < 0.3) { showToast('Stěna je příliš krátká'); return; }
...
// ============================
// SPLIT HANDLE (přetahování hranice nahoře/dole)
// ============================
...
function initPaletteDrag() {
  // Standardní palette items (pokud existují)
  ...
  // Mini palette items v dolním panelu (Sklad, Vstup/Výstup)
  document.querySelectorAll('.palette-item-mini[draggable]').forEach(item => {
    item.addEventListener('dragstart', (e) => {
      dragType = item.dataset.type;
      e.dataTransfer.setData('text/plain', dragType);
      e.dataTransfer.effectAllowed = 'copy';
      if (dom.ghost) {
        dom.ghost.style.display = 'block';
        dom.ghost.innerHTML = `<div style="background:${COLORS[dragType].stroke}33; border:2px solid ${COLORS[dragType].stroke}; border-radius:8px; padding:6px 12px; color:${COLORS[dragType].stroke}; font-size:12px;">${COLORS[dragType].label}</div>`;
        e.dataTransfer.setDragImage(dom.ghost, 40, 20);
      }
    });
    item.addEventListener('dragend', () => {
  ...
  dom.container.addEventListener('drop', (e) => {
    ...
    if (wsData && type === 'pracoviste') {
      try {
        ...
        const wsW = ws.w || sz.w;
        const wsH = ws.h || sz.h;
        const obj = createObject(type, world.x - wsW / 2, world.y - wsH / 2);
        if (obj) {
...
function initCanvasMouse() {
  ...
  dom.container.addEventListener('mousemove', (e) => {
    ...
    // Vertex hover — ukázat nápovědu rotace (Shift)
...
function zoomFit() {
  ...
  if (ww <= 0 || wh <= 0) { state.zoom = 1; state.panX = 50; state.panY = 50; updateTransform(); return; }

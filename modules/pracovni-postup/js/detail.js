...
const state = {
  ...
  workflow: null,
  operations: [],
  bomTree: [],         // Stromová struktura BOM (root items s children)
  totalCount: 0,       // Celkový počet položek ve všech úrovních
  goodsCache: {},      // Cache: goodsId → detail data
  currentTab: 'sestava',
  loading: false,
...
function locName(val) {
  if (!val) return '';
  if (typeof val === 'object') return val['2'] || val['1'] || Object.values(val)[0] || '';
  return String(val);
}
...
function extractBomFromGoods(goodsData, parentCode) {
  const items = [];
  if (!goodsData || !goodsData.workflow || !goodsData.workflow.operations) return items;

  const operations = goodsData.workflow.operations.sort((a, b) => {
  ...
  operations.forEach((op, opIdx) => {
    ...
    bomItems.forEach(bom => {
      ...
      items.push({
        ...
        children: [],  // Bude naplněno rekurzí
        expanded: false,
      });
    });
  });
...
async function loadFullBom(goodsId, parentCode, depth, visited) {
  ...
  if (depth === 0) {
    ...
    if (goodsData.workflow && goodsData.workflow.operations) {
      state.workflow = goodsData.workflow;
      state.operations = goodsData.workflow.operations.sort((a, b) =>
  ...
  const subItems = directItems.filter(i => i.hasSubWorkflow && i.id && !visited.has(i.id));
...
async function loadProductDetail(goodsId) {
  ...
  try {
    ...
    const name = locName(mainData.name);
    const code = mainData.code || '';

    dom.productName.textContent = name || 'Bez názvu';
    dom.productCode.textContent = code;
    document.title = `${code} ${name} | Pracovní postup`;

    if (mainData.workflow && mainData.workflow.operations) {
      state.workflow = mainData.workflow;
      state.operations = mainData.workflow.operations.sort((a, b) =>
        (a.position || 0) - (b.position || 0)
      );
    }
    ...
    dom.tabContent.innerHTML = `
      <div class="error-state">
        ...
        <button class="btn" onclick="loadProductDetail('${goodsId}')">Zkusit znovu</button>
      </div>`;
  }
}
...
function renderSestava() {
  ...
  function collectTypes(arr) { arr.forEach(i => { if (i.type) allTypes.add(i.type); if (i.children) collectTypes(i.children); }); }
  collectTypes(items);
...
// ancestors = pole boolean — true = na dané úrovni pokračuje vertikální čára (má sourozence pod sebou)
...
// ancestors = boolean[] — na pozici i je true pokud má předek na hloubce i další sourozence pod tímto uzlem
...
function toggleTreeNode(toggleEl, rowId) {
...
function buildOpTree(goodsId, visited) {
  if (!goodsId || visited.has(goodsId)) return [];
  visited.add(goodsId);

  const data = state.goodsCache[goodsId];
  if (!data || !data.workflow || !data.workflow.operations) return [];

  const ops = data.workflow.operations
    .slice()
  ...
  return ops.map((op, idx) => {
    ...
    // Typ pracoviště — COOPERATION = kooperace (Zinkovna, Lakovna, Krejčovství, Obrábění...)
    ...
    const bomItems = (op.billOfMaterialsItems || [])
      .map(bom => {
        ...
        if (!isExpandable) return null; // Přeskočit materiály a ostatní
...
function renderNormovane() {
  ...
  const productName = locName(state.product.name);
  const productCode = state.product.code || '';
  const productType = state.product.type ? (state.product.type.name || state.product.type.referenceName || '') : '';
...
function renderNormTreeRows(nodes, depth, ancestors) {
  ...
  nodes.forEach((node, idx) => {
    ...
    if (node.nodeType === 'operation') {
      ...
      let wfFinal = wfIcon;
      if (manualA) {
        stageHtml = `<span class="assigned-stage">${escapeHtml(manualA.stageName || manualA.stage || '')}</span>`;
      ...
      html += `
        <tr class="tree-row norm-goods-row depth-${depth}${manualA ? ' norm-row-assigned' : ''}" data-depth="${depth}" data-row-id="${rowId}">
          ...
          <td class="col-qty">${node.quantity !== '' ? node.quantity : ''}</td>
...
function toggleOperationBom(arrow, opIdx) {
...
function renderVizualizace() {
  ...
  //   — tag each sub-task with feedsIntoMainPos (which main op it feeds)
  // ═══════════════════════════════════════════════════════════════
  const tasks = [];
  let nextId = 0;
  let currentMainPos = null; // set while processing each main op
  ...
  function collectGoodsOps(goodsNode) {
    ...
    if (ops.length === 0) {
      ...
      if (manual && manual.norm > 0) {
        ...
        tasks.push({
          ...
          stage: manual.stageName || manual.stage || '?',
    ...
    let prevOpId = null;
    ops.forEach(op => {
      const secs = normToSeconds(op.normDuration, op.normUnit);
  ...
  opTree.forEach(mainOp => {
    const secs = normToSeconds(mainOp.normDuration, mainOp.normUnit);
    const deps = prevMainId !== null ? [prevMainId] : [];
    const isKoop = (mainOp.stageType || '').toUpperCase() === 'COOPERATION';

    currentMainPos = mainOp.position; // tag sub-tasks
  ...
  // ═══════════════════════════════════════════════════════════════
  // STEP 2: Backward schedule — plan from the end (as late as possible)
  // ═══════════════════════════════════════════════════════════════
  const taskMap = new Map(tasks.map(t => [t.id, t]));

  // 2a: Forward pass to determine makespan and workplace task ordering
  {
    ...
    while (remaining.size > 0 && safety++ < 50000) {
      ...
      for (const id of remaining) {
        ...
        if (!task.dependsOn.every(d => scheduled.has(d))) continue;
        const depEnd = task.dependsOn.length > 0
          ? Math.max(...task.dependsOn.map(d => taskMap.get(d).endSec))
          : 0;
  ...
  // 2b: Build successors map + workplace ordering from forward pass
  const successors = new Map(tasks.map(t => [t.id, []]));
  tasks.forEach(t => t.dependsOn.forEach(d => successors.get(d).push(t.id)));
  ...
  Object.values(wpTaskOrder).forEach(arr =>
    arr.sort((a, b) => taskMap.get(a).startSec - taskMap.get(b).startSec)
  );
  const wpNextTask = new Map();
  Object.values(wpTaskOrder).forEach(ids => {
    ids.forEach((id, i) => { if (i < ids.length - 1) wpNextTask.set(id, ids[i + 1]); });
  });

  // 2c: Backward pass — schedule each task as late as possible
  {
    ...
    while (remainingBack.size > 0 && safety++ < 50000) {
      ...
      for (const id of remainingBack) {
        ...
        // Latest possible end: earliest successor start, or makespan if no successors
        let latestEnd = succs.length > 0
          ? Math.min(...succs.map(s => taskMap.get(s).startSec))
          : makespan;

        // Workplace constraint: must end before next task on same workplace starts
        if (wpNextTask.has(id)) {
          latestEnd = Math.min(latestEnd, taskMap.get(wpNextTask.get(id)).startSec);
        }
  ...
  const mainColorMap = {};
  mainTasks.forEach((mt, i) => { mainColorMap[mt.position] = mt.isKoop ? '#e67e22' : MC[i % MC.length]; });

  // Group sub-tasks by workplace
  const wpMap = {};
  subTasks.forEach(t => { if (!wpMap[t.stage]) wpMap[t.stage] = []; wpMap[t.stage].push(t); });
  ...
  function renderGantt(zoomLevel) {
    ...
    mainTasks.forEach(mt => {
      ...
      const mainBotY = AXIS_H + ROW_H; // bottom of main row
      ...
      wps.forEach(wp => {
        ...
        const lastEndX = toPx(Math.max(...subs.filter(s => s.stage === wp).map(s => s.endSec)));
    ...
    if (timeline && labels) {
      timeline.addEventListener('scroll', () => { labels.scrollTop = timeline.scrollTop; });
    }
...
function getStageColor(stage, idx) {
  const colors = [
    '#6c8cff', '#27ae60', '#9b59b6', '#3498db', '#1abc9c',
    '#e74c3c', '#f39c12', '#2ecc71', '#e91e63', '#00bcd4',
  ];
  if (!stage) return colors[idx % colors.length];
  let hash = 0;
  for (let i = 0; i < stage.length; i++) hash = ((hash << 5) - hash + stage.charCodeAt(i)) | 0;
...
function applyFilters() {
  ...
  if (hasFilters) {
    // Zobrazit všechny tree-children-container
    document.querySelectorAll('tr.tree-children-container').forEach(tr => tr.style.display = '');
  }
...
function resetFilters() {
  document.querySelectorAll('.col-filter, .col-filter-select').forEach(el => el.value = '');
  applyFilters();
  collapseAll();
}
...
function collectNormStats(nodes, parentGoods) {
  ...
  for (const node of nodes) {
    if (node.nodeType === 'operation') {
      const seconds = normToSeconds(node.normDuration, node.normUnit);
      ...
      // Chyba: norma < 30s, ALE vyloučit pracoviště typu COOPERATION (Kooperace)
    ...
    // Varování: polotovar/výrobek bez pracovního postupu (nemá children = operace)
...
function openAssignModal(code, name) {
...
function confirmAssignment(code) {
...
function removeAssignment(code) {
...
// ---- Hromadné přiřazení ----
function toggleAllWarnings(checked) {
  document.querySelectorAll('.warn-check').forEach(cb => cb.checked = checked);
  updateBulkCount();
}
...
function switchBulkNormMode(mode, btn) {
  document.querySelectorAll('.bulk-norm-tab').forEach(t => t.classList.remove('active'));
  btn.classList.add('active');
  document.getElementById('bulk-norm-same').style.display = mode === 'same' ? '' : 'none';
  document.getElementById('bulk-norm-individual').style.display = mode === 'individual' ? '' : 'none';
...
function confirmBulkAssignment() {
  ...
  if (mode === 'same') {
    const norm = parseFloat(document.getElementById('assign-norm')?.value) || 0;
    if (norm <= 0) {
      const inp = document.getElementById('assign-norm');
    ...
    let hasError = false;
    inputs.forEach(inp => {
      const norm = parseFloat(inp.value) || 0;
      if (norm <= 0) { inp.style.borderColor = '#e74c3c'; hasError = true; return; }

/* ============================================
   renderer.js — Vykreslování SVG (simulace)
   ============================================ */
...
function renderBackgroundObject(parent, obj) {
  ...
  if (obj.points && obj.points.length >= 3) {
    ...
    text.setAttribute('x', bbox.minX + 1.5);
    text.setAttribute('y', bbox.minY + 2);
...
function renderRoutePath() {
  ...
  for (let i = 0; i < mappedOps.length - 1; i++) {
    ...
    // Šipka
    const line = svgEl('line');
    line.setAttribute('x1', from.floorX);
    line.setAttribute('y1', from.floorY);
    line.setAttribute('x2', to.floorX);
    line.setAttribute('y2', to.floorY);
...
function renderTokens() {
  ...
  state.tokens.forEach(token => {
    if (token.state === 'done' && !token.visible) return;
...
function highlightStation(obj, active) {

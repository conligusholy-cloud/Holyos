...
// GET /api/compounder/slots-free — nadcházející výrobní týdny (jeden slot = jeden stroj).
// Vrací nejbližší volné týdny; už založené a prázdné ProductionSloty použije, obsazené/blokované vynechá,
// zbytek nabídne jako „nový" týden (ref NEW|start|end), který se v DB vytvoří až při rezervaci.
router.get('/slots-free', requireAuth, async (req, res, next) => {
  try {
    const weeksAhead = 14;
    const persisted = await prisma.productionSlot.findMany({
      where: { status: { notIn: ['blocked', 'closed'] } },
      include: { _count: { select: { assignments: true } }, blocks: { select: { id: true } } },
    });
    const byStart = {};
    persisted.forEach(function (s) { byStart[new Date(s.start_date).toISOString().slice(0, 10)] = s; });
    const base = new Date(); base.setHours(0, 0, 0, 0);
    const dow = (base.getDay() + 6) % 7;
    const monday0 = new Date(base); monday0.setDate(base.getDate() - dow);
    const out = [];
    for (let w = 0; w < weeksAhead; w++) {
      const mon = new Date(monday0); mon.setDate(monday0.getDate() + w * 7);
      const fri = new Date(mon); fri.setDate(mon.getDate() + 4);
      if (fri < base) continue;
      const key = mon.toISOString().slice(0, 10);
      const label = 'Týden ' + _isoWeek(mon);
      const ps = byStart[key];
      if (ps) {
        if ((ps.blocks && ps.blocks.length) || (ps._count && ps._count.assignments || 0) > 0) continue; // obsazený / blokovaný
        out.push({ ref: String(ps.id), name: ps.name || label, start_date: ps.start_date, end_date: ps.end_date });
      } else {
        out.push({ ref: 'NEW|' + key + '|' + fri.toISOString().slice(0, 10), name: label, start_date: mon, end_date: fri });
      }
    }
    res.json(out);
  } catch (err) { next(err); }
...
router.post('/leads/:id(\\d+)/create-sales-order', requireAuth, async (req, res, next) => {
  try {
    ...
    // Výrobní sloty k rezervaci (ref z /api/compounder/slots-free), jeden na kus.
    // ref je buď číslo (existující ProductionSlot), nebo 'NEW|<startISO>|<endISO>' (týden se založí).
    const slotRefs = Array.isArray(b.slots) ? b.slots.map(function (x) { return String(x || '').trim(); }).filter(Boolean) : [];
    ...
    if (slotRefs.length) {
      ...
      for (let si = 0; si < slotRefs.length; si++) {
        const ref = slotRefs[si];
        let slotId = null;
        try {
          if (/^\d+$/.test(ref)) {
            slotId = parseInt(ref, 10);
          } else if (ref.indexOf('NEW|') === 0) {
            const parts = ref.split('|');
            const sd = new Date(parts[1]), ed = new Date(parts[2]);
            if (!isNaN(sd.getTime()) && !isNaN(ed.getTime())) {
              const created = await prisma.productionSlot.create({ data: { name: 'Výrobní týden ' + parts[1], start_date: sd, end_date: ed, status: 'open' } }).catch(function () { return null; });
...
// GET /api/compounder/my-funnel — stejný trychtýř, ale nad kontakty přiřazenými
// přihlášenému obchodníkovi (všechny zdroje, bez testovacích).
router.get('/my-funnel', requireAuth, async (req, res, next) => {
  try {
    const meId = (req.user && req.user.person) ? req.user.person.id : null;
    if (!meId) return res.json({ total: 0 });
    const leads = await prisma.compounderLead.findMany({
      where: { owner_person_id: meId, is_test: false },
    ...
    const estDeals = Math.round(openOnPortal * convRatePortal);
    res.json({
      total, bySource, accessSent, onPortal, engaged, converted, lost, openOnPortal, estDeals,
      avgDaysToPortal, medDaysToPortal,
      convRatePortalPct: Math.round(convRatePortal * 1000) / 10,
      sentToPortalPct: accessSent ? Math.round(onPortal / accessSent * 1000) / 10 : 0,
    });
...
// POST /api/compounder/my-funnel-analysis — AI analýza trychtýře přihlášeného obchodníka.
router.post('/my-funnel-analysis', requireAuth, async (req, res, next) => {
  try {
    if (!process.env.ANTHROPIC_API_KEY) return res.status(503).json({ error: 'AI není nakonfigurováno (chybí ANTHROPIC_API_KEY).' });
    const meId = (req.user && req.user.person) ? req.user.person.id : null;
    if (!meId) return res.status(400).json({ error: 'Chybí obchodník.' });
    const leads = await prisma.compounderLead.findMany({
      where: { owner_person_id: meId, is_test: false },
      select: { id: true, source: true, status: true, created_at: true, access_last_sent_at: true, access_sent_count: true },
    });
    ...
    let pricing = null;
    try { const cs = (await getSetting(COMPOUNDING_SETTINGS_KEY, { type: 'json', defaultValue: COMPOUNDING_SETTINGS_DEFAULT })) || {}; pricing = { pricelist_eur: cs.pricelist || null, discount: cs.discount || null }; } catch (e) { /* volitelné */ }

    const Anthropic = require('@anthropic-ai/sdk');
    const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
    const model = process.env.COMPOUNDER_LOCATION_MODEL || 'claude-sonnet-4-6';
    const sys = 'Jsi obchodní kouč jednoho obchodníka, který prodává samoobslužné prádlomaty (Compounder) z přiřazených kontaktů. Dostaneš JEHO trychtýř (celkem kontaktů → odeslán přístup do portálu → byli na portále → aktivní zájem → obchod), časové metriky (od kdy má kontakty, tempo za týden), týdenní řadu a ceník strojů. Vyhodnoť konkrétně a povzbudivě, ale realisticky: jak si vede, kde ztrácí nejvíc kontaktů, KDY se dá čekat jeho první obchod (i při 0 obchodech odhadni z počtu na portále a běžné konverze), jaké tempo má držet (kolik kontaktovat, kolik přístupů poslat týdně) a krátkou predikci na 1–3 měsíce. Odpověz POUZE platným JSON bez markdownu: {"summary":"<2-4 věty>","first_deal":"<kdy a proč>","required_pace":"<konkrétní tempo>","prediction":"<1-3 měs výhled>","actions":["<konkrétní krok>","..."]}. Piš česky, přímo obchodníkovi (tykej), konkrétně.';
    const usr = 'Můj trychtýř + časová osa (JSON):\n' + JSON.stringify(funnel) + '\n\nTýdenní řada (leads/access/portal):\n' + JSON.stringify(series) + (pricing ? ('\n\nCeník strojů (EUR):\n' + JSON.stringify(pricing)) : '');
    const msg = await client.messages.create({ model, max_tokens: 1200, system: sys, messages: [{ role: 'user', content: usr }] });
    let text = (msg && msg.content && msg.content[0] && msg.content[0].text) || '';
    text = text.replace(/^```(json)?/i, '').replace(/```\s*$/, '').trim();
    let j; try { j = JSON.parse(text); } catch (e) { j = { summary: text.slice(0, 1200), first_deal: '', required_pace: '', prediction: '', actions: [] }; }
    res.json({
      ok: true, funnel, series,
      analysis: {
        summary: String(j.summary || '').slice(0, 1200),
        first_deal: String(j.first_deal || '').slice(0, 600),
        required_pace: String(j.required_pace || '').slice(0, 600),
        prediction: String(j.prediction || '').slice(0, 800),
        actions: Array.isArray(j.actions) ? j.actions.slice(0, 8).map((a) => String(a).slice(0, 300)) : [],
      },
    });
  } catch (err) {
...
// Krátký kód pro zkrácený odkaz /s/<kód> → base36(id) + '.' + 8 znaků podpisu. Šetří délku SMS.
function makeShortCode(leadId) {
  const id = Number(leadId);
  return id.toString(36) + '.' + hmacSig('short:' + id).slice(0, 8);
}
function verifyShortCode(code) {
  const s = String(code || '');
  const i = s.lastIndexOf('.');
  if (i < 1) return null;
  const id = parseInt(s.slice(0, i), 36);
  if (!Number.isInteger(id) || id <= 0) return null;
  if (!safeEqStr(s.slice(i + 1), hmacSig('short:' + id).slice(0, 8))) return null;
  return id;
}

...
function twimlApology() {
  return (
    '<?xml version="1.0" encoding="UTF-8"?>\n' +
    '<Response>\n' +
    '  <Say voice="Polly.Jitka" language="cs-CZ">Omlouvám se, teď se nedaří nikoho zastihnout. Ozveme se vám co nejdříve. Na shledanou.</Say>\n' +
    '  <Hangup/>\n' +
    '</Response>'
  );
}
...
// Vlastní telefonní číslo dané linky (z mapy voice.line_numbers: číslo → linka).
// Pro Infolinku vrátí číslo Infolinky, aby se z něj volalo při přepojení.
async function numberForLine(line) {
  try {
    const get = settings && settings.getSetting; if (!get) return '';
    let map = await get('voice.line_numbers');
    if (typeof map === 'string') { try { map = JSON.parse(map); } catch (_) { map = null; } }
    if (map && typeof map === 'object') {
      for (const k of Object.keys(map)) { if (normLine(map[k]) === normLine(line)) return k; }
    }
...
function twimlDial(numbers, timeout, ctx) {
  ...
  const c = ctx || {};
  const cidNum = e164((c.callerId || VOICE_DEFAULT_FROM || ''));   // volající číslo = číslo linky (Infolinky)
  const callerId = cidNum ? ` callerId="${xmlAttr(cidNum)}"` : '';
  const q = [`q=${tail}`, `t=${t}`];
...
async function resolveTransferPlan(mode, targetId, line) {
  ...
  if (mode === 'outbound' && targetId && prisma.voiceCampaignTarget) {
    ...
    // 0) Dnešní směna: v pracovní době osoba „v práci" → hlavní → záložní operátor
    //    (vše z rozpisu daného dne; mají přednost před statickými čísly).
    try {
  ...
  // Volající číslo pro přepojení = vlastní číslo linky (Infolinky). Fallback na default.
  let callerId = '';
  if (mode !== 'outbound') { callerId = await numberForLine(line); }
  if (!callerId) callerId = VOICE_DEFAULT_FROM || '';
  return {
    enabled,
    numbers: queue,
...
// (přes tlačítko „Zavolat zpět" v mobilním přehledu). Přidá záznam do callback_log.
router.post('/calls/:id/callback', requireAuth, async (req, res, next) => {
  try {
    if (!prisma.voiceCall) return res.status(404).json({ error: 'Nedostupné' });
    const call = await prisma.voiceCall.findUnique({ where: { id: req.params.id }, select: { id: true, callback_log: true } });
    if (!call) return res.status(404).json({ error: 'Hovor nenalezen' });
    let pid = null;
    try {
      const per = (req.user && req.user.person) || null;
      if (per && per.id) pid = per.id;
    } catch (_) { /* nevadí */ }
    const name = (req.user && (req.user.displayName || req.user.username)) || 'Neznámý';
    const log = Array.isArray(call.callback_log) ? call.callback_log.slice() : [];

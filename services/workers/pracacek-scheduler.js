// =============================================================================
// HolyOS — Pracáček scheduler (worker skeleton)
// =============================================================================
// Plánované akce pro Pracáček:
...
//   require('./services/workers/pracacek-scheduler').start();
...
async function handleMorningGenerate() {
  console.log('[pracacek-scheduler] 06:30 — generuj DailyPlan pro aktivní kolegy');
  try {
    ...
    const peopleWithDevice = await prisma.person.findMany({
      where: {
        active: true,
        pracacek_devices: { some: { active: true } },
      },
      select: { id: true },
    });
    ...
    console.log(`[pracacek-scheduler] Vygenerováno ${created} nových plánů`);
  } catch (err) {
    console.error('[pracacek-scheduler] handleMorningGenerate selhal:', err);
  }
}

async function handleMorningPush() {
  console.log('[pracacek-scheduler] 07:00 — ranní push');
  try {
    const today = startOfToday();
    ...
    console.error('[pracacek-scheduler] handleMorningPush selhal:', err);
  }
}

async function handleEveningPush() {
  console.log('[pracacek-scheduler] 16:30 — večerní reflexe push');
  try {
    const today = startOfToday();
    const plans = await prisma.dailyPlan.findMany({
    ...
    console.error('[pracacek-scheduler] handleEveningPush selhal:', err);
  }
}
...
function start() {
  if (process.env.PRACACEK_SCHEDULER_DISABLED === '1') {
    console.log('[pracacek-scheduler] DISABLED via PRACACEK_SCHEDULER_DISABLED=1');
    return;
  }
  if (_tickHandle) return;
  console.log('[pracacek-scheduler] start — tick každých 60 s, markery 06:30 / 07:00 / 16:30');

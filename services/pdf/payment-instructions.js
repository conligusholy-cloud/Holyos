...
async function renderItem(item, bank, tr) {
  ...
  const eurqr = (bank.eur && bank.eur.iban) ? await qrDataUrl(epcString({ name: bank.eur.name, iban: bank.eur.iban, bic: bank.eur.bic, amount: item.amount, message: item.label })) : null;
  ...
  const foreign = (bank.eur && bank.eur.iban) ? `
    <div class="col">
      ...
      <table class="kv">
        ...
        <tr><td>${esc(tr.amount)}</td><td><b>${money(item.amount)} EUR</b></td></tr>
      </table>
...
async function generatePaymentInstructionsPdf({ title, buyer, items, bank, note, lang }) {
  const tr = L(lang);
  const parts = [];
  for (const it of (items || [])) parts.push(await renderItem(it, bank || {}, tr));

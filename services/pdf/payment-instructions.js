...
async function renderItem(item, bank, tr, eurRate) {
  const cur = item.currency || 'CZK';
  const czkIban = czkIbanOf(bank.czk);
  const czqr = czkIban ? await qrDataUrl(buildSpaydString({ iban: czkIban, amount: item.amount, currency: cur === 'EUR' ? 'EUR' : 'CZK', vs: item.vs, message: item.label })) : null;
  // Zahraniční platba je vždy v EUR — přepočet z CZK dle kurzu (CZK za 1 EUR).
  const rate = (eurRate && eurRate > 0) ? eurRate : 25;
  const eurAmount = (cur === 'EUR') ? Number(item.amount) : (Number(item.amount) / rate);
  const eurRounded = Math.round(eurAmount * 100) / 100;
  ...
  const domestic = bank.czk ? `
    <div class="col">
      ...
      <table class="kv">
        ...
        <tr><td>${esc(tr.amount)}</td><td><b>${money(item.amount)} ${esc(cur)}</b></td></tr>
      </table>
  ...
  const foreign = (bank.eur && bank.eur.iban) ? `
    <div class="col">
      ...
      <table class="kv">
        ...
        <tr><td>${esc(tr.amount)}</td><td><b>${money(eurRounded)} EUR</b><div style="font-size:10px;color:#999">${money(item.amount)} ${esc(cur)} · ${esc(tr.rate)} ${rate.toLocaleString('cs-CZ')} CZK/EUR</div></td></tr>
      </table>
  ...
  return `<div class="item">
    ...
    <div class="idue">${esc(tr.due)} <b>${czDate(item.due)}</b> · ${esc(tr.amount)}: <b>${money(item.amount)} ${esc(cur)}</b></div>
    <div class="cols">${domestic}${foreign}</div>
  </div>`;
}

async function generatePaymentInstructionsPdf({ title, buyer, items, bank, note, lang, eurRate }) {
  const tr = L(lang);
  const parts = [];
  for (const it of (items || [])) parts.push(await renderItem(it, bank || {}, tr, eurRate));

...
export function updateTransform() {
    if (!dom.objectLayer || !dom.connectionLayer || !dom.snapLayer || !dom.drawLayer)
        return;
    ...
    if (gridSmall && gridLarge && dom.gridRect && dom.zoomDisplay) {
        ...
        if (pathSmall)
            pathSmall.setAttribute('d', `M ${gs} 0 L 0 0 0 ${gs}`);
        ...
        if (pathLarge)
            pathLarge.setAttribute('d', `M ${gs5} 0 L 0 0 0 ${gs5}`);
...
function renderDiamondObject(g, obj, strokeColor, fillColor) {
...
function renderRectangleObject(g, obj, strokeColor, fillColor) {
    const rect = svgEl('rect');
    rect.setAttribute('width', String(obj.w));
    rect.setAttribute('height', String(obj.h));
    rect.setAttribute('rx', obj.type === 'cesta' ? '0.2' : '0.3');
...
export function resizeSVG() {
    ...
    if (isNaN(state.panY))
        state.panY = 40;
    if (isNaN(state.zoom) || state.zoom <= 0)
        state.zoom = 1;
    updateTransform();
}

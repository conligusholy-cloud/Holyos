...
export function showProperties(id) {
    ...
    <div class="prop-group">
      ...
      <div class="prop-color">
        ${COLOR_SWATCHES.map(c => `<div class="prop-color-swatch ${obj.color === c ? 'active' : ''}" style="background:${c}" onclick="window.__module__.updateColor('${c}')"></div>`).join('')}
      </div>
    </div>`;
    // Návaznosti
...
export function updateVertex(objId, index, axis, value) {
    window.__module__.updateVertex(objId, index, axis, value);
}
export function updateEdgeDistance(objId, fromIndex, newDist) {
    window.__module__.updateEdgeDistance(objId, fromIndex, newDist);
}
export function removeVertex(objId, index) {
    window.__module__.removeVertex(objId, index);
}
export function updateGateProp(objId, wallId, gateId, key, value) {
    window.__module__.updateGateProp(objId, wallId, gateId, key, value);
}
// ---- Zamykání objektů ----
export function toggleLock(objId) {
    window.__module__.toggleLock(objId);
}
...
export function updateTitleBar() {
    window.__module__?.updateTitleBar?.();
}

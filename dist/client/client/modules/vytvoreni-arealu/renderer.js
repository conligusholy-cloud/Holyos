...
export function initDom() {
    ...
    dom.objectLayer = document.getElementById('object-layer');
    dom.connectionLayer = document.getElementById('connection-layer');
    dom.snapLayer = document.getElementById('snap-layer');
    dom.labelLayer = document.getElementById('label-layer');
    dom.drawLayer = document.getElementById('draw-layer');
    dom.gridRect = document.getElementById('grid-rect');
...
export function updateTransform() {
    ...
    const gridSmallPath = gridSmall.querySelector('path');
    ...
    const gridLargeRect = gridLarge.querySelector('rect');
    gridLargeRect.setAttribute('width', gs5.toString());
    gridLargeRect.setAttribute('height', gs5.toString());
    const gridLargePath = gridLarge.querySelector('path');
...
function renderEntranceMarker(g, obj, entrance) {
    ...
    if (entrance.type === 'vjezd' || isBoth) {
        ...
        arrow.setAttribute('points', `${tipX},${tipY} ${baseX + ex * arrowW},${baseY + ey * arrowW} ${baseX - ex * arrowW},${baseY - ey * arrowW}`);
...
// ---- Náhled kreslení ----
export function renderDrawPreview(mouseWorld) {
...
export function getPolygonBBox(pts) {
...
export function isPointInPolygon(px, py, pts) {
    let inside = false;
...
export function renderEntrancePlacePreview(mouseWorld) {
    dom.snapLayer.innerHTML = '';
    if (!state.entrancePlaceMode || !mouseWorld)
        return;
...
export function renderWallDrawPreview(mouseWorld) {
    dom.snapLayer.innerHTML = '';
    if (!state.wallDrawMode || !state.wallDrawStart || !mouseWorld)
        return;
...
export function renderWallSnapHover(snap) {
    dom.snapLayer.innerHTML = '';
    if (!snap)
        return;
...
export function renderGatePlacePreview(obj, wallId, projected) {
    dom.snapLayer.innerHTML = '';
    if (!obj || !obj.walls || wallId === null)
        return;

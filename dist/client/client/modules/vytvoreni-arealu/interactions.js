...
import { createObject, createPolygonObject, findObjectAt, selectObject, deleteObject, duplicateObject, moveVertex, findNearestArealEdge, addWall, projectOntoWall, addGate, addRoomLabel, rotatePolygon, addEntrance } from './objects.js';
import { pushUndo, undo, redo } from './history.js';
import { deselectAll, getRotateCenter } from './properties.js';
...
// ============================
export function startWallDrawMode(objId) {
    cancelAllModes();
...
function projectPointOnEdge(wx, wy, x1, y1, x2, y2) {
    ...
    if (lenSq < 0.001)
        return { px: x1, py: y1, t: 0, dist: Math.sqrt((wx - x1) ** 2 + (wy - y1) ** 2), edgeLen: 0 };
...
export function confirmWallPoint() {
...
export function finishWall(endPoint) {
...
function updateWallDistInput(distValue, maxLen, step) {
    ...
    if (!box) {
        showWallDistInput();
        box = document.getElementById('wall-dist-input');
    }
...
export function updateDrawStatus() {
    ...
    if (state.gatePlaceMode) {
        ...
        let msg;
        if (state.wallDrawStart)
            msg = 'Stěna — klikni na obvod/stěnu pro koncový bod  |  Escape';
        else
            msg = 'Stěna — klikni na obvod nebo stěnu pro počáteční bod  |  Escape';

...
import { snapToGrid, getPolygonBBox, getPolygonCentroid, isPointInPolygon } from './renderer.js';
...
// ============================================
export function addEntrance(objId, edgeIndex, t1, t2, type, wallId) {
    pushUndo();
    const obj = state.objects.find(o => o.id === objId);
    if (!obj || !obj.points)
        return;
...
// ============================================
export function updateWallName(objId, wallId, name) {
    const obj = state.objects.find(o => o.id === objId);
    if (!obj || !obj.walls)
        return;
    const wall = obj.walls.find(w => w.id === wallId);
...
export function updateWallPoint(objId, wallId, which, axis, value) {
    const obj = state.objects.find(o => o.id === objId);
    if (!obj || !obj.walls)
        return;
    const wall = obj.walls.find(w => w.id === wallId);
    ...
    if (wall.locked)
        return;
    pushUndo();
    const key = (which === 'start' ? (axis === 'x' ? 'x1' : 'y1') : (axis === 'x' ? 'x2' : 'y2'));
    wall[key] = value;
    renderAll();
    showProperties(objId);
}
// Nastav délku stěny — posune koncový bod podél směrového vektoru
export function updateWallLength(objId, wallId, newLength) {
    const obj = state.objects.find(o => o.id === objId);
    if (!obj || !obj.walls)
        return;
    const wall = obj.walls.find(w => w.id === wallId);
...
export function updateWallAngle(objId, wallId, angleDeg) {
    const obj = state.objects.find(o => o.id === objId);
    if (!obj || !obj.walls)
        return;
    const wall = obj.walls.find(w => w.id === wallId);
...
function findNearestHallEdge(obj, wx, wy) {
...
export function snapWallPerpendicular(objId, wallId) {
    const obj = state.objects.find(o => o.id === objId);
    if (!obj || !obj.walls)
        return;
    const wall = obj.walls.find(w => w.id === wallId);
...
function findConnectedWalls(walls, startWallId, eps = 0.15) {
...
export function updateWallDistFromCorner(objId, wallId, distFromStart) {
    const obj = state.objects.find(o => o.id === objId);
    if (!obj || !obj.walls)
        return;
    const wall = obj.walls.find(w => w.id === wallId);
...
export function toggleRoomLocked(objId, wallId) {
    const obj = state.objects.find(o => o.id === objId);
    if (!obj || !obj.walls)
        return;
    const wall = obj.walls.find(w => w.id === wallId);
    ...
    const group = findConnectedWalls(obj.walls, wallId);
    pushUndo();
    const anyLocked = group.some((w) => w.locked);
    for (const w of group) {
        w.locked = !anyLocked;
    }
    renderAll();

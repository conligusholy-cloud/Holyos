/* ============================================
   ...
   ============================================ */
import { FactorifyBrowser } from './factorify-api.js';
import { MENU, TOP_ITEMS, buildEntityResolver } from './factorify-menu.js';
const state = {
    activeSlug: null,
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        state.entityResolver = buildEntityResolver(FactorifyBrowser.entities.map(e => ({ name: e.name, endpointUrl: e.endpointUrl, label: e.label })));
...
function renderNav() {
    ...
    container.onclick = (e) => {
        ...
        // Category toggle
        const catHeader = target.closest('.fy-cat-header');
        if (catHeader && catHeader.dataset.cat) {
            toggleCategory(catHeader.dataset.cat);
            return;
        }
        // Nav item click
        const navItem = target.closest('.fy-nav-item, .fy-nav-top-item');
...
function bindTableEvents(container) {
    ...
    if (tableWrapper) {
        tableWrapper.addEventListener('click', (e) => {
            const target = e.target;
            if (target.classList.contains('fy-ref'))
                return;
            // Sort click on th
            const th = target.closest('thead tr:first-child th');
    ...
    // Search
    const searchInput = container.querySelector('#fy-search-input');
    ...
    const pageSizeSelect = container.querySelector('#fy-page-size');
    if (pageSizeSelect) {
...
function renderDetail(container) {
    ...
    container.querySelectorAll('.fy-detail-tab').forEach(tab => {
        tab.addEventListener('click', () => {
            state.detailTab = tab.dataset.tab || 'OBECNÉ';
            renderMain();
        });
    });
}
// ---- Format values ----
function formatCell(val, _fieldName) {
    if (val === null || val === undefined)
        return '<span class="fy-null">—</span>';
    if (typeof val === 'boolean')
        return val ? '<span class="fy-bool-true">✓</span>' : '<span class="fy-bool-false"></span>';
    // Reference object
    ...
    if (Array.isArray(val))
        return '<span class="fy-array">[' + val.length + ']</span>';
    // DateTime
...
function formatDetailVal(val, key) {
    if (val === null || val === undefined)
        return '<span class="fy-null">—</span>';
    if (typeof val === 'boolean')
        return val ? '<span class="fy-bool-true">Ano</span>' : '<span class="fy-bool-false">Ne</span>';
    if (typeof val === 'object' && val !== null && !Array.isArray(val)) {
        const targetEntity = val.classNameForRequestHash || '';
        const targetId = val.id || '';
    ...
    if (Array.isArray(val)) {
        if (val.length === 0)
            return '<span class="fy-null">prázdné</span>';
...
async function openEntityDirect(entityName) {
...
async function openRecord(entityName, recordId) {
...
function doSearch() {
    const input = document.getElementById('fy-search-input');
...
// ---- Expose to window ----
window.__openRecord = openRecord;
window.__openEntity = openEntityDirect;

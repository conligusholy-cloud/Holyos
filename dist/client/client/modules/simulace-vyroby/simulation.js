...
import { renderTokens, highlightStation } from './renderer.js';
...
export function pauseSimulation() {
    ...
    if (state.simAnimFrame)
        cancelAnimationFrame(state.simAnimFrame);
    updateSimButtons();
}
...
export function setSimSpeed(value) {
    ...
    if (speedDisplay)
        speedDisplay.textContent = state.simSpeed + '×';
}
...
function moveToken(token, dt) {
    ...
    if (dist < 0.3) {
        ...
        if (op && op.floorObj)
            highlightStation(op.floorObj, true);
        return;
    }
    ...
    if (op) {
        const key = op.stageId || op.stageName;
        const util = state.metrics.stationUtil[key];
        if (util) {
...
function processToken(token, op, dt) {
    ...
    if (token.progress >= 1) {
        ...
        if (op.floorObj)
            highlightStation(op.floorObj, false);
        // Další krok
        token.currentStep++;
...
function updateSimUI() {
    ...
    state.tokens.forEach(token => {
        ...
        const pct = token.state === 'done' ? 100 : Math.min(100, Math.round(((token.currentStep + token.progress) / state.route.length) * 100));
...
function updateRouteHighlight() {
    ...
    state.tokens.forEach(t => {
        if (t.currentStep > maxStep)
            maxStep = t.currentStep;
    });
...
function updateMetricsUI() {
    ...
    if (prodEl)
        prodEl.textContent = formatSimTime(totalProd / state.simBatchSize);
    const moveEl = document.getElementById('metric-move-time');
    if (moveEl)
        moveEl.textContent = formatSimTime(totalMove / state.simBatchSize);
    const waitEl = document.getElementById('metric-wait-time');
    if (waitEl)
        waitEl.textContent = formatSimTime(totalWait / state.simBatchSize);
    // Vytížení pracovišť
...
function calculateFinalMetrics() {
    ...
    stationEntries.forEach(s => {
        const busy = s.busy;
        if (busy < maxBusy * 0.5)
            return; // zobrazit jen nejvytíženější

...
class PersistentStorageImpl {
    SERVER_URL;
    _cache = {};
    _serverAvailable = null; // null = neznámo, true/false
    constructor() {
    ...
    async migrateFromLocalStorage(keys) {
        const keyArray = Array.isArray(keys) ? keys : [keys];
        const results = await Promise.all(keyArray.map(async (key) => {
            try {
...
// ==========================================
export const PersistentStorage = new PersistentStorageImpl();

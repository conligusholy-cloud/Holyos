...
function renderSidebar(activeModule) {
  ...
  modules.forEach(function(m) {
    ...
    var href = m.active ? (basePath + 'modules/' + m.id + '/index.html') : '#';
...
function getBasePath() {
  ...
  if (path === '/' || path.match(/^\/index\.html/)) return './';
  // If we're inside modules/xxx/something.html
  var match = path.match(/\/modules\//);
  if (match) {
    var before = path.substring(0, path.indexOf('/modules/'));
    return before + '/';
  }
  return './';
}

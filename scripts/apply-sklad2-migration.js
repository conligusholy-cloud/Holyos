// HolyOS — Sklad 2.0 | Aplikace vygenerované migrace na DB + zápis do historie
//
// Použití: node scripts/apply-sklad2-migration.js <migration_name>
// Např.:   node scripts/apply-sklad2-migration.js 20260423143720_sklad_2_pwa_tisk
//
// Kroky:
...
if (!migName) {
  ...
  console.error('Použití: node scripts/apply-sklad2-migration.js <migration_name>');
  process.exit(1);
}
...
if (!fs.existsSync(migFile)) {
  console.error(`CHYBA: ${migFile} neexistuje. Spustil jsi nejdřív .\\scripts\\migrate-sklad2.ps1?`);
  process.exit(1);
}
...
try {
  ...
  console.log('\n' + '='.repeat(70));
  console.log('HOTOVO — migrace sklad-2-pwa-tisk je applied.');
  console.log('='.repeat(70));
  console.log('\nDalší krok: seed default ZPL šablon a 2 tiskáren:');
  console.log('   node scripts/seed-sklad2.js');
} catch (e) {
